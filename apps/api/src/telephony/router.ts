...
export function createTelephonyRouter(maybeDeps: TelephonyDeps | undefined) {
  ...
  return router({
    numbers: router({
      list: route({ permission: 'phoneNumber:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(z.object({}).strict())
        .query(async ({ ctx }) => numbers.listNumbers(actorOf(ctx).subject.orgId)),

      search: route({ permission: 'phoneNumber:purchase', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(
          z
            .object({
      ...
      purchase: route({ permission: 'phoneNumber:purchase', stepUp: true, feature: { flag: 'telephony', display: 'Voice & Messaging' } })
      ...
      release: route({ permission: 'phoneNumber:release', stepUp: true, feature: { flag: 'telephony', display: 'Voice & Messaging' } })
      ...
      setRoute: route({ permission: 'phoneNumber:purchase', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
    ...
    calls: router({
      list: route({ permission: 'call:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
      ...
      place: route({ permission: 'call:place', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(
          z
            .object({
    ...
    recordings: router({
      list: route({ permission: 'recording:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
       ...
       */
      download: route({ permission: 'recording:export', stepUp: true, quotaClass: 'expensive', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
      ...
      transcript: route({ permission: 'recording:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(z.object({ recordingId: z.string().uuid() }).strict())
        .query(async ({ ctx, input }) =>
     ...
     */
    messages: router({
      threads: route({ permission: 'sms:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(z.object({ limit: z.number().int().min(1).max(100).default(50) }).strict())
        .query(async ({ ctx, input }) =>
          messages.listThreads(actorOf(ctx).subject.orgId, deps(), { limit: input.limit }),
        ),

      list: route({ permission: 'sms:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(
          z
            .object({
      ...
      send: route({ permission: 'sms:send', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(
          z
            .object({
              to: PhoneNumberSchema,
     ...
     */
    cards: router({
      recordings: route({ permission: 'recording:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(z.object({ cardId: CardIdSchema }).strict())
        .query(async ({ ctx, input }) =>
          recordingCards.listCardRecordings(actorOf(ctx), { cardId: input.cardId }),
        ),

      attach: route({ permission: 'recording:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
      ...
      detach: route({ permission: 'recording:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(z.object({ recordingId: z.string().uuid(), cardId: CardIdSchema }).strict())
        .mutation(async ({ ctx, input }) => {
    ...
    spend: router({
       ...
       * personal data.
       */
      current: route({ permission: 'phoneNumber:read', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
        .input(z.object({}).strict())
        .query(async ({ ctx }) => {
       ...
       */
      report: route({ permission: 'recording:read', quotaClass: 'expensive', feature: { flag: 'telephony', display: 'Voice & Messaging' } })
