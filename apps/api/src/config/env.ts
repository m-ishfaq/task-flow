...
export const EnvSchema = z
  .object({
    ...
    /* A SEVENTH role, for the platform-operator console (Phase 12 §3.7,
       migration 0032's own header). taskflow_platform_admin reaches
       identity.orgs/identity.memberships across every tenant through two
       permissive RLS policies, and holds no grant at all on any product
       table. Optional, matching every other cross-tenant role above: an
       instance that only serves ordinary requests does not need it;
       `withPlatformAdminScope` throws rather than silently falling back to
       the application role, which cannot see identity.orgs across every
       tenant and would silently list nothing. */
