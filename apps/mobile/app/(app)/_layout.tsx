...
export default function AppLayout() {
  ...
  useEffect(() => {
    ...
    const resolved = resolveRememberedOrg(
      ...
      // walked past.
      //
      // `orgStatus === 'active'` is the identical fix one level up, for the
      // ORG itself (Phase 12 Wave 1's platform console) rather than this
      // caller's own membership — without it, a remembered id naming an org
      // an operator had since suspended would auto-select it here and every
      // query on the guarded screens would answer ORG_SUSPENDED instead,
      // the same silent-walk-past regression this file already caught once
      // for the membership case.
      //
      ...
      orgs.data
        .filter((org) => org.membershipStatus === 'active' && org.orgStatus === 'active')
