/**
 * Slash commands — a verbatim port of `apps/web/src/features/chat/
 * slash-commands.ts` (Wave 3, ai/phase-5-chat.md §5). The logic is copied
 * unchanged, not re-derived, because the file has no DOM/React import of
 * its own to begin with — the same "ported... logic unchanged" relationship
 * `chat.ts`'s own `groupMessages` already has to web's `grouping.ts`. Found
 * missing entirely by the 2026-08-22 chat-parity audit: nothing on native
 * had ever named this file, and `channel/[channelId].tsx`'s composer sent
 * `/topic ...` and `/leave` as literal text.
 *
