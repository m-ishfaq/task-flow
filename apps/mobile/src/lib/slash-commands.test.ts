...
/**
 * Ported verbatim from `apps/web/src/features/chat/slash-commands.test.ts`
 * — the module under test is a byte-for-byte copy, so its test suite is
 * too; a native-only bug here would mean the port drifted, which is
 * exactly what running the same suite against it catches.
 *
