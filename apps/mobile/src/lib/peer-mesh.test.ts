 ...
 * impossible to see (ai/phase-13-webrtc.md §3.5) — ported from `apps/web/
 * src/features/rtc/peer-mesh.test.ts`, same cases, same reasoning; only
 * the fakes differ, since this file also has to stand in for
 * `RTCIceCandidate`/`RTCSessionDescription` (both are injected — see
 * `peer-mesh.ts`'s own header on why `react-native-webrtc` cannot be
 * imported here as a VALUE at all, even for a test — only `import type`,
 * identically to that file's own only-type import).
 *
 * A mesh that glares does not throw. Both peers sit in `have-local-offer`,
 * the audio never arrives, and it reproduces only when two people join
 * within a few hundred milliseconds of each other — so it looks like a
 * flaky network and gets "fixed" by asking the user to try again.
 */

interface FakeIceEvent {
  candidate: { toJSON: () => unknown } | null;
}
...
const CONFIGURATION: RtcConfiguration = { iceServers: [], iceTransportPolicy: 'all' };
...
function build(selfId: string) {
  ...
  const mesh = new PeerMesh({
    ...
    createIceCandidate: (init: RtcIceCandidateInit) => init as unknown as RTCIceCandidate,
    createSessionDescription: (init: RtcSessionDescriptionInit) =>
      init as unknown as RTCSessionDescription,
  });
