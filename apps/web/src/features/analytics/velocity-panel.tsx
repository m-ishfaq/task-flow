...
export function VelocityPanel({ orgId }: { readonly orgId: string }) {
  ...
  const points = data ?? [];
  if (points.length === 0) {
    return <Empty title="No velocity data" description="Transitions will appear as cards move through statuses." />;
  }
