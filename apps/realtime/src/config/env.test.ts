...
const valid = {
  ...
  DATABASE_REALTIME_URL: 'postgresql://taskflow_realtime:secret@localhost:5432/taskflow',
...
describe('parseEnv', () => {
  it('accepts a complete environment and applies defaults', () => {
    ...
    expect(env.REALTIME_PORT).toBe(3001);
  });
  ...
  it('rejects a variable that looks like ours but is misspelled', () => {
    ...
    expect(() => parseEnv({ ...valid, REALTIME_PORTT: '3001' })).toThrow(/REALTIME_PORTT/);
  });
   ...
   * file header — this gateway verifies, it never mints or issues its own
   * challenge tokens), but both are spelled correctly and legitimately
   * present in a developer's shared root `.env` alongside `JWT_PUBLIC_KEY`.
   * Before this app was taught about them, a real boot against a real `.env`
   * refused to start over two perfectly correct variables — the exact
   * failure mode this file's own `KNOWN_VARIABLES` comments document for
   * every `DATABASE_*` sibling role, just not caught here until a real
   * boot found it, because every other test in this file constructs `Env`
   * with a tidy fixture rather than a realistic shared `.env`.
   */
