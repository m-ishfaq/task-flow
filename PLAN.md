...
| 6   | **Docs** 🔨 _backend done, no UI_       | Spaces, page tree, Hocuspocus + Yjs, inline comments, versions, templates, inherited permissions, publish. Waves 1–4 (tree/auth foundation, live sync + the three recovery paths, comments/suggestions/backlinks, publish/PDF/templates) all shipped on `apps/api` + `apps/collab`. **`apps/web` has no Docs UI at all** — a person cannot create a space or open a page yet; see `ai/phase-6-docs.md`'s status header. Full spec: [ai/phase-6-docs.md](ai/phase-6-docs.md)                          | 8 wks   |
