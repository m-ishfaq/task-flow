...
| 6.5  | **UI Polish & Design System** _draft_   | Extracts `packages/ui` from the thirteen call sites that currently hand-roll their own Modal/DropdownMenu/Popover/Select/Checkbox around Radix (Work, Chat, Docs, Auth all reimplement independently), fixes the resulting drift, and brings Chat/Docs/Auth/Admin/Notifications up to the optimistic-mutation/skeleton/toast bar Work alone cleared in Phase 3.5 Wave 1. No new product surface — the deliverable is fewer divergent implementations of things that already exist, plus whatever the accessibility audit turns up. Six waves: audit; design tokens; `packages/ui` extraction; cross-surface consistency pass; accessibility hardening; responsive layout (sized separately, scope pending). Full spec: [ai/phase-6.5-ui-polish.md](ai/phase-6.5-ui-polish.md), drafted 2026-08-08, not yet approved. | 5–7 wks |
